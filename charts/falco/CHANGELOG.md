...
* Debugfs is now mounted by default when using legacy eBPF or modern eBPF drivers (see `driver.ebpf.debugfs` and `driver.modernEbpf.debugfs` respectively) or when `driver.kind` is set to `auto` 
